...
public class SummaryDistribution {
	...
	/// notes: replace ? & (?) with " and its native range is likely to be x // its
	/// native range is not known, but is thought to be x
	private static final Map<Pattern, String> transformerDoubtful = new ImmutableMap.Builder<Pattern, String>()
			.put(Pattern.compile("\\(\\?\\)"), "").put(Pattern.compile("\\?^\\)"), "").build();

	private static final Map<Pattern, String> transformerAbreviations = new ImmutableMap.Builder<Pattern, String>()
			.put(Pattern.compile("I\\."), "Island").put(Pattern.compile("Is\\."), "Islands")
			.put(Pattern.compile("Trop\\."), "Tropical").put(Pattern.compile("Arch\\."), "Archipelago")
			.put(Pattern.compile("Co\\."), "County").put(Pattern.compile("Distr\\."), "District")
			.put(Pattern.compile("Pen\\."), "Peninsula").put(Pattern.compile("Subtrop\\."), "Subtropical")
			.put(Pattern.compile("Rep\\."), "Republic").put(Pattern.compile("Mts\\."), "Mountains")
			.put(Pattern.compile("NC\\."), "N. Central").put(Pattern.compile("SC\\."), "S. Central")
			.put(Pattern.compile("WC\\."), "W. Central").put(Pattern.compile("EC\\."), "E. Central")
			.put(Pattern.compile("\\sC\\."), " Central").put(Pattern.compile("^C\\."), "Central")
			.put(Pattern.compile("Throughout"), "throughout")
			// strip trailing punctuation
			.put(Pattern.compile("\\.$"), "").build();
