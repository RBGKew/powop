        	...
        	if(t.getParent() != null) {
        		for(Taxon child : t.getParent().getChildren()) {
        			if(child.getIdentifier().equals(t.getIdentifier())) {
        				logger.debug(t.getIdentifier() + " removed successfully from parent? " + t.getParent().getChildren().remove(child));
        				break;
        			}
        		}
        	...
        	if(t.getAccepted() != null) {
        		for(Taxon synonym : t.getAccepted().getSynonyms()) {
                    if(synonym.getIdentifier().equals(t.getIdentifier())) {
                    	logger.debug(t.getIdentifier() + " removed successfully from accepted name? " + t.getAccepted().getSynonyms().remove(synonym));
                    	break;
                    }
        		}
                t.setAccepted(null);
