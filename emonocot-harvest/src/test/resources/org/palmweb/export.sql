...
select IFNULL(taxon.lsid_lsid,'') as id,  IFNULL(IF(ref.uuid IS NULL,NULL,concat('http://www.palmweb.org/?q=cdm_dataportal/reference/',ref.uuid)),'') as identifier, ifnull(ref.titleCache,'') as bibliographicCitation, CASE ref.refType WHEN 0 THEN 'article-journal' WHEN 1 THEN 'book' WHEN 2 THEN 'chapter' WHEN 3 THEN 'cd' WHEN 4 THEN 'database' WHEN 5 THEN 'misc' WHEN 7 THEN 'journal' WHEN 10 THEN 'personal_communication' WHEN 11 THEN 'series' WHEN 15 THEN 'webpage'  END as type, ifnull(ref.title,'') as title, ifnull(ref.volume,'') as volume, ifnull(ref.series,'') as series, ifnull(ref.pages,'') as pages, ifnull(ref.referenceAbstract,'') as abstract, ifnull(substr(ref.datepublished_start,1,4),'') as date,ifnull(inref.title,'') as source, ifnull(authors.titleCache,'') as creator from reference as ref left join reference as inref on (ref.inreference_id= inref.id) join agentbase as authors on (ref.authorTeam_id = authors.id) join originalsourcebase as source on (source.citation_id = ref.id) join descriptionelementbase as element on (source.sourcedObj_id = element.id and source.sourcedObj_type = 'eu.etaxonomy.cdm.model.description.TextData') join descriptionbase as description on (element.indescription_id = description.id) join taxonbase as taxon on (taxon.id = description.taxon_id) limit 1;
...
select ref.id as id,  IFNULL(IF(ref.uuid IS NULL,NULL,concat('http://www.palmweb.org/?q=cdm_dataportal/reference/',ref.uuid)),'') as identifier, ref.titleCache, CASE ref.refType WHEN 0 THEN 'article-journal' WHEN 1 THEN 'book' WHEN 2 THEN 'chapter' WHEN 3 THEN 'cd' WHEN 4 THEN 'database' WHEN 5 THEN 'misc' WHEN 7 THEN 'journal' WHEN 10 THEN 'personal_communication' WHEN 11 THEN 'series' WHEN 15 THEN 'webpage'  END as type, ref.title, ref.volume, ref.seriespart, ref.pages, ref.referenceAbstract as abstract, substr(ref.datepublished_start,1,4) as date, inref.title as source, authors.titleCache as authors, author.titlecache as author from reference as ref left join reference as inref on (ref.inreference_id= inref.id) join agentbase as authors on (ref.authorTeam_id = authors.id) left join agentbase_agentbase as a_a on (authors.id = a_a.AgentBase_id) left join AgentBase as author on (a_a.teammembers_id = author.id) limit 3;