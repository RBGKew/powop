...
public class EoLTaxonItemConverterTest {
     ...
     */
    @Test
    public final void testConvertWithNewObject() {
        EasyMock.expect(taxonService.load(EasyMock.eq("identifier"))).andThrow(
                new DataRetrievalFailureException("Could not find object"));
        taxonItem = new EoLTaxonItem();
        taxonItem.setIdentifier("identifier");
        EasyMock.replay(taxonService, imageConverter, textDataConverter,
                referenceConverter);

        Taxon taxon = converter.convert(taxonItem);
        assertNotNull("Taxon should not be null", taxon);

        EasyMock.verify(taxonService, imageConverter, textDataConverter,
                referenceConverter);
    }
}
