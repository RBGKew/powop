...
public class ChecklistWebserviceControllerTest {
	...
	public void testPing() {
		...
		ModelAndViewAssert.assertViewName(checklistWebserviceController.ping(),"rdfResponse");
		EasyMock.verify(taxonDao);
	}

	@Test
	public void testSearch() {
		EasyMock.expect(taxonDao.search(EasyMock.eq("Poa annua"))).andReturn(new ArrayList<Taxon>());
		EasyMock.replay(taxonDao);
		ModelAndView modelAndView = checklistWebserviceController.search("Poa annua");
		ModelAndViewAssert.assertViewName(modelAndView,"rdfResponse");
		ModelAndViewAssert.assertModelAttributeAvailable(modelAndView, "result");
		EasyMock.verify(taxonDao);
	}

	@Test
	public void testGet() {
		EasyMock.expect(taxonDao.get(EasyMock.eq("123"))).andReturn(new Taxon());
		EasyMock.replay(taxonDao);
	...
	@Test
	public void testHandleInvalidTaxonIdentifier() {
		EasyMock.replay(taxonDao);
		ModelAndView modelAndView = checklistWebserviceController.handleInvalidTaxonIdentifier(new DataRetrievalFailureException("Invalid identifier 123"));
		ModelAndViewAssert.assertViewName(modelAndView,"exception");
		ModelAndViewAssert.assertModelAttributeAvailable(modelAndView, "exception");
		EasyMock.verify(taxonDao);
	}
